...
function StudentDashboardContent() {
  ...
  const loadData = async () => {
    ...
    // Load applications
    const appsRes = await getMyCreditApplication();
    if (appsRes.success) {
      ...
      const statsData = {
        total: apps.length,
        pending: apps.filter(a => a.ct_status === 'submitted' || a.ct_status === 'under_review' || a.ct_status === 'awaiting_sme').length,
        approved: apps.filter(a => a.ct_status === 'approved').length,
        rejected: apps.filter(a => a.ct_status === 'rejected').length,
        draft: apps.filter(a => a.ct_status === 'draft').length,
      };
      setStats(statsData);
    }

    // Load appointments
    const apptRes = await getAppointmentHistory();
    if (apptRes.success) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-lg p-6 text-white">
        ...
        <p className="text-indigo-100">Manage your credit transfer applications and track your progress</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-semibold text-gray-800 mb-2">Apply for Credit Transfer</h3>
              <p className="text-gray-600 text-sm">Submit a new credit transfer application</p>
            </div>
            <PlusIcon className="h-12 w-12 text-indigo-600" />
          </div>
        </Link>
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 text-sm">Schedule a meeting with your coordinator</p>
            </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <p className="font-medium text-gray-800">
                        Application #{app.ct_id || app.id}
                      </p>
                      <p className="text-sm text-gray-600 mt-1">
                        {formatDate(app.createdAt)}
                      </p>
                    </div>
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(app.ct_status || app.status)}`}>
                      {(app.ct_status || app.status || "draft").replace(/_/g, " ").toUpperCase()}
                    </span>
                  </div>
                </div>
              ))}
              ...
              {appointments.map((appt) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <p className="font-medium text-gray-800">
                        {appt.coordinator?.name || "Coordinator"}
                      </p>
                      <p className="text-sm text-gray-600 mt-1">
                    ...
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                      appt.status === 'scheduled' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                    }`}>
